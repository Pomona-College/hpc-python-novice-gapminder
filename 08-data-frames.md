...
## .iloc vs .loc: Don't Mix Them Up

It's easy to confuse `iloc` (index location, numeric position) with `loc` (label-based location, by name). Remember: `iloc` uses row/column numbers (0, 1, 2...), while `loc` uses labels (country names, year columns). Using the wrong one is a common source of bugs when working with DataFrames.
