...
## Use Descriptive Names and Write Docstrings

Always use meaningful names for functions (not `f()` or `process()`). Even better, write a docstring explaining what the function does, what arguments it expects, and what it returns. Your future self will thank you, and anyone reviewing your code will understand it faster. Docstrings are also used by Python's `help()` function.
