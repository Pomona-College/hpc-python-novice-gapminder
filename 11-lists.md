...
## append vs extend: A Common Mistake

`append()` adds an entire item to the end of a list (which could be another list, making nested lists). `extend()` unpacks another list and adds each item individually. This is a frequent source of confusion. If you want to combine two lists, use `extend()`. If you want to add a single item, use `append()`.
